...
cfg=json.load(open(sys.argv[1]))
def valid(tag):
    return isinstance(tag,str) and 0 < len(tag) <= 128 and all(ch.isalnum() or ch in '._-' for ch in tag)
...
PY
)" || die "primary WARP/inbound discovery failed"
while IFS='=' read -r key value; do
  case "$key" in
    PRIMARY_WARP_TAG) PRIMARY_WARP_TAG="$value" ;;
    ACCESS_INBOUND_TAG) ACCESS_INBOUND_TAG="$value" ;;
  esac
done <<<"$discovery"
...
XRAY_AUTODIRECT_INBOUND_TAG=$ACCESS_INBOUND_TAG
XRAY_AUTODIRECT_PRIMARY_WARP_TAG=$PRIMARY_WARP_TAG
EOF
