...
# Auto-Direct observes only the local Xray access log. 3x-ui fresh installs
# commonly ship with access=none. Enable it through LoggerService only: this
...
python3 - "$ACTIVE_CFG" "$DB" "$ACCESS_LOG" "$log_tmp" "$template_tmp" <<'PY'
import json, os, sqlite3, sys
cfg_path, db_path, access_path, out_cfg, out_tpl = sys.argv[1:]
active=json.load(open(cfg_path, encoding='utf-8'))
...
open(out_cfg,'w',encoding='utf-8').write(json.dumps(active,indent=2,ensure_ascii=False)+'\n')
open(out_tpl,'w',encoding='utf-8').write(json.dumps({'id':row_id,'old':old_template,'new':json.dumps(template,ensure_ascii=False)},ensure_ascii=False))
os.chmod(out_cfg,os.stat(cfg_path).st_mode & 0o777)
PY
"$XRAY_BIN" run -test -c "$log_tmp" >/dev/null || die "access-log config did not validate"
pid_before="$(pgrep -o -f '^bin/xray-linux-amd64 -c bin/config.json
...
python3 - "$DB" "$template_tmp" "$log_tmp" "$ACTIVE_CFG" <<'PY'
import json, os, sqlite3, sys
db_path, template_path, staged_cfg, active_cfg=sys.argv[1:]
p=json.load(open(template_path,encoding='utf-8'))
con=sqlite3.connect(db_path)
try:
    with con:
        changed=con.execute("UPDATE settings SET value=? WHERE id=? AND value=?",(p['new'],p['id'],p['old']))
        if changed.rowcount!=1: raise SystemExit('xrayTemplateConfig changed concurrently; access-log update cancelled')
    os.replace(staged_cfg,active_cfg)
finally:
    con.close()
PY
"$XRAY_BIN" api restartlogger -s 127.0.0.1:62789 -t 3 >/dev/null ||
  die "Xray LoggerService refresh failed; config was backed up for recovery"
sleep 1
pid_after="$(pgrep -o -f '^bin/xray-linux-amd64 -c bin/config.json
note "Obtaining wgcf for a fresh, independent WARP identity"
arch="$(dpkg --print-architecture)"
case "$arch" in amd64) wgarch=amd64 ;; arm64) wgarch=arm64 ;; *) die "unsupported architecture: $arch" ;; esac
asset_url="$(curl -fsSL https://api.github.com/repos/ViRb3/wgcf/releases/latest | python3 -c '
...
if [[ "$APPLY" == 1 ]]; then
 ...
 || true)"
[[ "$pid_before" == "$pid_after" ]] || die "Xray PID changed while refreshing LoggerService"
systemctl is-active --quiet x-ui.service || die "x-ui became inactive while refreshing LoggerService"
